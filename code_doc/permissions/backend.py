...
class CodedocPermissionBackend(object):
    ...
    def _populate_permissions(self, user, obj):
        ...
        return set(codename for codename
                   in self.objects_permission_handlers[type(obj)]
    ...
    def has_perm(self, user_obj, perm, obj=None):
        ...
        # This is delegated to the object itself, otherwise anonymous users won't get
...
def create_tables():
    ...
    for cls in (m for m
                in config.get_models(include_auto_created=False)
        ...
        if(hasattr(cls, '_meta') and hasattr(cls._meta, 'permissions')):
            for permission_name, _ in cls._meta.permissions:
                ...
                if(hasattr(cls, handler_function)):
                    ...
                    logger.warning('[permissions][backend] "%s" does not handle object permission "%s"',
                                   cls, permission_name)

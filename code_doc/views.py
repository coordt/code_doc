...
class ProjectSeriesAddView(PermissionOnObjectViewMixin, CreateView):
  ...
  .. note:: in order to be able to add a series, the user should have the 'code_doc.project_series_add' permission
  ...
  def get_project_from_request(self, request, *args, **kwargs):
    ...
    try:
      ...
      logger.warning('[ProjectSeriesAddView] non existent project with id %s', kwargs['project_id'])
      return None
...
class ProjectSeriesDetailsView(PermissionOnObjectViewMixin, DetailView):
  ...
  .. note:: the user should have the 'series_view' and the 'series_artifact_view' permissions on the series object
  ...
  def get_series_from_request(self, request, *args, **kwargs):
    ...
    try:
      ...
      logger.warning('[ProjectSeriesDetailsView] non existent project with id %d', kwargs['project_id'])
      return None

    try:
      series = project.series.get(pk=kwargs['series_id'])
    except ProjectSeries.DoesNotExist:
      logger.warning('[ProjectSeriesDetailsView] non existent series with id %d', kwargs['series_id'])
      return None
...
class ProjectSeriesArtifactEditionFormsView(PermissionOnObjectViewMixin):
  ...
  def get_project_from_request(self, request, *args, **kwargs):
    ...
    try:
      ...
      logger.warning('[ProjectSeriesArtifactEditionFormsView] non existent project with id %d', self.kwargs['project_id'])
      return None

    try:
      current_series = current_project.series.get(pk=self.kwargs['series_id'])
    except ProjectSeries.DoesNotExist:
      logger.warning('[ProjectSeriesArtifactEditionFormsView] non existent series for project "%s" with series.id "%s"', current_project, self.kwargs['series_id'])
      return None
  ...
  def get_success_url(self):
    return reverse('project_revision',
                   kwargs={'project_id': self.object.project_series.project.pk, 'series_id': self.object.project_series.pk})

...
class ProjectSeriesArtifactTest(TestCase):
    def setUp(self):
        ...
        self.new_series = ProjectSeries.objects.create(
        ...
        self.imgfile = StringIO.StringIO(
            "GIF87a\x01\x00\x01\x00\x80\x01\x00\x00\x00\x00ccc,\x00\x00\x00\x00\x01\x00\x01\x00\x00\x02\x02D\x01\x00;"
        )
        self.imgfile.name = "test_img_file1.gif"
        self.imgfile1 = StringIO.StringIO(
            "GIF87a\x11\x00\x01\x00\x80\x01\x00\x00\x00\x00ccc,\x00\x00\x00\x00\x01\x00\x01\x00\x00\x02\x02D\x01\x00;"
        )
        self.imgfile1.name = "test_img_file1.gif"
        self.imgfile2 = StringIO.StringIO(
            "GIF87a\x10\x00\x01\x00\x80\x01\x00\x00\x00\x00ccc,\x00\x00\x00\x00\x01\x00\x01\x00\x00\x02\x02D\x01\x00;"
        )
        self.imgfile2.name = "test_img_file2.gif"

    def create_artifact_file(
        self, file_to_add=None, is_documentation=False, tar_basename=None
    ):
    ...
    def test_project_series_artifact_wrong_id(self):
        ...
        initial_path = reverse(
            self.path, args=[self.project.id, self.new_series.id + 1]
        )
    ...
    def test_project_series_artifact_possible_for_admins(self):
        ...
        _ = User.objects.create_superuser(
            username="admin", email="bla@bla.com", password="admin"
        )
        response = self.client.login(username="admin", password="admin")
        self.assertTrue(response)

        response = self.client.get(
            reverse(self.path, args=[self.project.id, self.new_series.id]), follow=False
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_send_new_artifact_no_login(self):
        ...
        response = self.client.post(
            initial_path, {"name": "fred", "attachment": self.imgfile}, follow=True
        )
    ...
    def test_send_new_artifact_no_login_no_follow(self):
        ...
        initial_path = reverse(self.path, args=[self.project.id, self.new_series.id])
        response = self.client.post(
            initial_path, {"name": "fred", "attachment": self.imgfile}, follow=False
        )
    ...
    def test_send_new_artifact_with_login_malformed(self):
        ...
        response = self.client.post(
            ...
            {
                "name": "fred"
            },  # , 'attachment': self.imgfile}, # attachement missing while required
            follow=False,
        )
    ...
    def test_send_new_artifact_with_login(self):
        ...
        self.assertEqual(
            Revision.objects.count(), 2
        )  # self.revision and revision 'blah'
    ...
    def test_get_all_artifacts_json(self):
        ...
        json_artifact_path = reverse(
            "api_get_artifacts", args=[self.project.id, self.new_series.id]
        )
        ...
        self.assertEqual(
            Revision.objects.count(), 2
        )  # self.revision and revision 'blah1'
        self.assertEqual(Branch.objects.count(), 1)
    ...
    def test_revision_and_branch_creation_on_artifact_upload(self):
        ...
        try:
            ...
            self.fail(
                "[Revision.DoesNotExist] The Revisions returned no object from the get query"
            )
        except Revision.MultipleObjectsReturned:
            self.fail(
                "[Revision.MultipleObjectsReturned] The Revisions returned more than one object from the get query"
            )
        except Exception:
        ...
        try:
            ...
            self.fail(
                "[Branch.DoesNotExist] The Branches returned no object from the get query"
            )
        except Branch.MultipleObjectsReturned:
            self.fail(
                "[Branch.MultipleObjectsReturned] The Branches returned more than one object from the get query"
            )
        except Exception:
            self.fail("Unexpected Exception in get query")
            raise
    ...
    def test_multiple_upload_of_artifacts_for_same_branch(self):
        ...
        try:
            ...
            self.fail(
                "[DoesNotExist] One of the Revisions returned no object from the get query"
            )
        except Revision.MultipleObjectsReturned:
            self.fail(
                "[MultipleObjectsReturned] One of the Revisions returned more than one object from the get query"
            )
        except Exception:
            self.fail("Unexpected Exception in get query")
            raise
    ...
    def test_create_documentation_artifact(self):
        ...
        with tempfile.NamedTemporaryFile(
            dir=settings.USER_UPLOAD_TEMPORARY_STORAGE, suffix=".tar.bz2"
        ) as f:
            # create a temporary tar object
            tar = tarfile.open(fileobj=f, mode="w:bz2")
    ...
    def test_check_artifact_consistency_on_upload(self):
        ...
        self.assertFormError(
            response, "form", field=None, errors="The submitted file is invalid"
        )
    ...
    def test_documentation_entry_point_check_relative_paths(self):
        ...
        f, source_file = self.create_artifact_file(
            is_documentation=True, tar_basename="./"
    ...
    def test_prune_directories_on_remove(self):
        ...
        with tempfile.NamedTemporaryFile(
            ...
            new_artifact.delete()
            self.assertFalse(
                os.path.exists(os.path.abspath(get_deflation_directory(new_artifact)))
            )
            self.assertFalse(
                os.path.exists(
                    os.path.abspath(os.path.dirname(new_artifact.full_path_name()))
                )
            )
            self.assertTrue(os.path.exists(get_deflation_directory(new_artifact2)))
            ...
            with tempfile.NamedTemporaryFile(
                dir=os.path.dirname(new_artifact2.full_path_name())
            ) as ftemp:
                ...
                self.assertFalse(
                    os.path.exists(
                        os.path.abspath(get_deflation_directory(new_artifact))
                    )
                )
                self.assertFalse(
                    os.path.exists(
                        os.path.abspath(get_deflation_directory(new_artifact2))
                    )
                )
                self.assertFalse(
                    os.path.exists(os.path.abspath(new_artifact2.full_path_name()))
                )
                ...
                self.assertTrue(
                    os.path.exists(
                        os.path.abspath(
                            os.path.join(
                                get_deflation_directory(new_artifact2), os.pardir
                            )
                        )
                    )
                )

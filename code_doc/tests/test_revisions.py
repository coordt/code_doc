...
class RevisionTest(TestCase):
    ...
    def setUp(self):
        ...
        self.revision1 = Revision.objects.create(revision="1", project=self.project)
        self.revision2 = Revision.objects.create(revision="2", project=self.project)
        self.revision3 = Revision.objects.create(revision="3", project=self.project)

        self.branch_master = Branch.objects.create(
            name="master", nb_revisions_to_keep=4
        )
        self.branch_develop = Branch.objects.create(
            name="develop", nb_revisions_to_keep=8
        )
    ...
    def test_cannot_add_artifact_to_serie_from_another_project(self):
        ...
        series_p2 = ProjectSeries.objects.create(
            series="12345", project=project2, release_date=datetime.datetime.now()
        )
    ...
    def test_artifact_promotion_no_revision_deletion(self):
        ...
        """
        rev3_artifact1 = Artifact.objects.create(
            project=self.project,
            revision=self.revision3,
            md5hash="4",
            artifactfile=self.test_file,
        )
        rev3_artifact2 = Artifact.objects.create(
            project=self.project,
            revision=self.revision3,
            md5hash="5",
            artifactfile=self.test_file,
        ...
        stable_series = ProjectSeries.objects.create(
            series="stable", project=self.project, release_date=datetime.datetime.now()
        )
    ...
    def test_artifact_promotion_by_assigning_no_revision_deletion(self):
        ...
        """
        rev3_artifact = Artifact.objects.create(
            project=self.project,
            revision=self.revision3,
            md5hash="4",
            artifactfile=self.test_file,
    ...
    def test_artifact_and_revision_deletion_on_no_more_referencing_series(self):
        ...
        try:
            ...
            self.fail(
                "[Artifact.DoesNotExist] The Artifacts returned no object from the get query"
            )
        except Artifact.MultipleObjectsReturned:
            self.fail(
                "[Artifact.MultipleObjectsReturned] The Artifacts returned more than one object from the get query"
            )
        except:
            self.fail("Unexpected Exception in get query")
            raise
    ...
    def test_revision_persistance_if_artifact_is_referenced_by_different_series(self):
        ...
        other_series = ProjectSeries.objects.create(
            series="other", project=self.project, release_date=datetime.datetime.now()
        )
...
class RevisionViewTest(TestCase):
    ...
    def test_project_revision_view_no_restricted_series(self):
        ...
        # Revisions
        revision = Revision.objects.create(revision="revision", project=self.project)
        ...
        response = self.client.get(
            reverse(self.path, args=[self.project.id, revision.id])
        )
    ...
    def test_project_revision_view_with_restrictions_on_series(self):
        ...
        _ = User.objects.create_user(
            username="weak_user", password="weak_user", email="w@w.fr"
        )
        ...
        response = self.client.login(
            username="test_revision_user", password="test_revision_user"
        )
        self.assertTrue(response)
        response = self.client.get(
            reverse(self.path, args=[self.project.id, revision.id])
    ...
    def test_project_revision_view_empty_access(self):
        ...
        empty_revision = Revision.objects.create(
            revision="empty_revision", project=self.project
        )

        # Test non existing revision
        response = self.client.get(
            reverse(self.path, args=[self.project.id, empty_revision.id + 41])
        )
        self.assertEqual(response.status_code, 401)

        # Test existing revision but to wrong project
        response = self.client.get(
            reverse(self.path, args=[self.project.id + 41, empty_revision.id])
        )
        self.assertEqual(response.status_code, 401)

        # Test existing revision from anonymous user
        response = self.client.get(
            reverse(self.path, args=[self.project.id, empty_revision.id])
        )
    ...
    def test_project_revision_context(self):
        ...
        self.series1 = ProjectSeries.objects.create(
            series="series1",
            project=self.project,
            release_date=datetime.datetime.now(),
            is_public=True,
        )
        self.series2 = ProjectSeries.objects.create(
            series="series2",
            project=self.project,
            release_date=datetime.datetime.now(),
            is_public=True,
        ...
        self.revision = Revision.objects.create(
            revision="revision", project=self.project
        )
        ...
        self.art3 = Artifact.objects.create(
            project=self.project,
            revision=self.revision,
            md5hash="3",
            artifactfile=self.test_file,
        ...
        # Test the revision
        response = self.client.get(
            reverse(self.path, args=[self.project.id, self.revision.id])
        )
        self.assertEqual(response.status_code, 200)

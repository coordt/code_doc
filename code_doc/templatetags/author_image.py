...
def author_image(author_id, size = None):
  ...
  return {'uploaded_image': None, #author.image,
          ...
          'background_color': hash_string_to_color_hex(author.firstname + author.lastname, 200),
          'size' : size}

def hash_string_to_color_hex(str, brightness_limit):
  hashed = hashlib.md5(str.encode('utf-8')).hexdigest()[:6]
  r = int(hashed[0:2], 16)
  g = int(hashed[2:4], 16)
  b = int(hashed[4:6], 16)
